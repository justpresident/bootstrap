...
if config:
    class MainApp:
        ...
        def left_click(self, icon):
            if (config._sections['general']['command'] != None and
                 config._sections['general']['command'] != ''):
                os.system(config._sections['general']['command'])
            else:
                binfo = self.get_battery_info()
                notify = 'notify-send -i "battery" -t 1000 "{title}"'
                os.system(notify.format(title = binfo["tooltip"]))
